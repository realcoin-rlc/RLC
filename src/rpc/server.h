...
/**
 * RealCoin RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZrlcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzrlcseed(const UniValue& params, bool fHelp);
extern UniValue getzrlcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzrlc(const UniValue& params, bool fHelp);
extern UniValue dzrlcstate(const UniValue& params, bool fHelp);

...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 RLC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzRLCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzRLCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzRLCReset.connect(boost::bind(NotifyzRLCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzRLCReset.disconnect(boost::bind(NotifyzRLCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zrlcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}

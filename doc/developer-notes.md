...
- [Development guidelines](#development-guidelines)
    - [General RealCoin Core](#general-realcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
RealCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
RealCoin Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_realcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_realcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/realcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_realcoin.coverage/index.html`.
```
...
However, in open source software such as RealCoin Core, where everyone uses
...
pay attention to for reviewers of RealCoin Core code.

General RealCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: RealCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against RealCoin Core so that they can be integrated
...
file descriptors it will cause problems with RealCoin's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `realcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `realcoin-cli` and the GUI debug console use this table to determine how to
